...
public:
	...
	void Animate(long dt, bool On) {
		if(On) {
			...
			dt = dt%60;
			dt = dt - 30;
			BPointerPos = Point( position.x + 0.145*cos(-dt*PI/60), position.y + 0.145*sin(-dt*PI/60));
			LPointerPos = Point( position.x + 0.11*cos(-dt*PI/3600), position.y + 0.11*sin(-dt*PI/3600));
		}
	}
};
